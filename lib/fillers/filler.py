...
class Filler(pl.LightningModule):
    ...
    def _check_metric(metric, on_step=False):
        if not isinstance(metric, MaskedMetric):
            if "reduction" in inspect.getfullargspec(metric).args:
                ...
                metric_kwargs = dict()
            return MaskedMetric(metric, metric_kwargs=metric_kwargs)
        return deepcopy(metric)

    def _set_metrics(self, metrics):
        self.train_metrics = MetricCollection(
            {
                f"train_{k}": self._check_metric(m, on_step=True)
    ...
    def predict_batch(
        ...
        if preprocess:
            ...
            y_hat = self.forward(x, **batch_data, set_t=set_t)
        else:
            y_hat = self.forward(**batch_data, set_t=set_t)
        # Rescale outputs
        if postprocess:
    ...
    def on_train_epoch_start(self) -> None:
        ...
        for i, optimizer in enumerate(optimizers):
            ...
            self.log(
                f"lr_{i}", lr, on_step=False, on_epoch=True, logger=True, prog_bar=False
            )

    def configure_optimizers(self):
        cfg = dict()
        optimizer = self.optim_class(
            self.trainer.model.parameters(), **self.optim_kwargs
        )

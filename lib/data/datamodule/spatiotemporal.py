...
class SpatioTemporalDataModule(pl.LightningDataModule):
    ...
    def __init__(
        ...
        self.scale_exogenous = (
            ensure_list(scale_exogenous) if scale_exogenous is not None else None
        )
        # data loaders
    ...
    def setup(self, stage=None):
        if self.scale:
            ...
            train_mask = (
                self.torch_dataset.mask.numpy()[self.train_slice]
                if "mask" in self.torch_dataset
                else None
            )
            scaler = (
                self.get_scaler()(scaling_axis)
                .fit(train, mask=train_mask, keepdims=True)
                .to_torch()
            )
    ...
    def train_dataloader(self, shuffle=True, batch_size=None):
        if self.samples_per_epoch is not None:
            sampler = RandomSampler(
                self.trainset, replacement=True, num_samples=self.samples_per_epoch
            )
            return self._data_loader(
                self.trainset, False, batch_size, sampler=sampler, drop_last=True
            )

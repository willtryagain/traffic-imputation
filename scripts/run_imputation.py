...
def run_experiment(args):
    ...
    # instantiate dataset
    dataset_cls = (
        GraphImputationDataset if has_graph_support(model_cls) else ImputationDataset
    )
    ...
    early_stop_callback = EarlyStopping(
        monitor="val_mse", patience=args.patience, mode="min"
    )
    checkpoint_callback = ModelCheckpoint(
        dirpath=logdir, save_top_k=1, monitor="val_mse", mode="min"
    )
